...
{
    public static class AccessoryScreenshotExporter
    {
        public static void Export()
        {
            ...
            if (camera == null)
                throw new InvalidOperationException("Digital Halo demo has no MainCamera");
            ...
            try
            {
                ...
                string output = Path.Combine(repository, "docs", "images",
                    "digital-halo-demo.png");
                Directory.CreateDirectory(Path.GetDirectoryName(output));
                File.WriteAllBytes(output, screenshot.EncodeToPNG());
                Debug.Log("[SabaAccessory example] Screenshot: " + output);
            }
            finally
            {
